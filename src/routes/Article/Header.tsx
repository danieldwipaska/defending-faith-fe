    ...
    <div>
      <div className="grid grid-cols-6 py-3 px-3 bg-gray-200">
        <div>
          <button
            onClick={() => {
              if (props?.from === menu.dashboard) {
                navigate(`/${menu.dashboard}`);
              } else if (props?.from === menu.apologetics) {
                navigate(`/${menu.apologetics}`);
              } else if (props?.from === menu.exegesis) {
                navigate(`/${menu.exegesis}`);
              } else {
                navigate(`/`);
              }
            }}
        ...
        <div className="col-span-4 self-center">
          <p className="text-base text-center">{props?.from}</p>
        </div>
        <div></div>
      </div>
    </div>
